...
{
    std::vector<SemaphoreStage> emptyWait;
    std::vector<CommandBuffer> cmdBuffers{cmdBuffer};
    std::vector<SemaphoreStage> emptySignal;
    return Submitting(queue, SubmissionBatchInfo{std::move(emptyWait), std::move(cmdBuffers), std::move(emptySignal)});
}
