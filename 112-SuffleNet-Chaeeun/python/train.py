...
def get_args():
    ...
    parser.add_argument('--eval-input-dir', type=str, default=None, help='directory of images to classify in eval mode')
...
def main():
    ...
    # Prepare validation loader only for training/validation path
    val_dataprovider = None
    if not args.eval:
    ...
    # Resolve eval weights early and align model-size when using bundled weights
    resume_path = args.eval_resume
    if args.eval:
        if (not resume_path) or (not os.path.exists(resume_path)):
            default_w = os.path.join(os.path.dirname(__file__), 'ShuffleNetV2+.Small.pth.tar')
            if os.path.exists(default_w):
                resume_path = default_w
                # Ensure model size matches small weights
    ...
    # Eval-only path: classify raw images under eval-input-dir
    if args.eval:
        # Load weights if provided or resolved
        if resume_path and os.path.exists(resume_path):
            checkpoint = torch.load(resume_path, map_location=None if use_gpu else 'cpu')
            load_checkpoint(model, checkpoint)
        else:
            logging.warning('No valid checkpoint found for eval; using randomly initialized weights.')

        # Determine eval input directory
        if args.eval_input_dir is None:
            # Default to ../data relative to this file
            args.eval_input_dir = os.path.normpath(os.path.join(os.path.dirname(__file__), '..', 'data'))

        run_simple_image_eval(model, device, args.eval_input_dir)
        return
...
def run_simple_image_eval(model, device, image_dir):
    # Collect images (jpg/png/jpeg)
    patterns = ['*.jpg', '*.jpeg', '*.png', '*.bmp']
    image_paths = []
    for p in patterns:
        image_paths.extend(glob.glob(os.path.join(image_dir, p)))

    if not image_paths:
        print(f'No images found in {image_dir}')
        return

    # Transform: resize->center crop->to BGR tensor
    ...
    with torch.no_grad():
        for img_path in sorted(image_paths):
            ...
            # Print only filename and predicted class index (no scores)
            print(f'{os.path.basename(img_path)} -> {pred}')

def load_checkpoint(net, checkpoint):
    from collections import OrderedDict

    # Unwrap to raw dict of weights
    state = checkpoint.get('state_dict', checkpoint)
    # Determine whether current model expects 'module.' prefix (DataParallel)
